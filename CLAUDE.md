...
- **Kill and Destroy are separate ticks.** `DestroyUpdateLogic.Update(dt)` (`State/StateParts/Destroy/Logic/DestroyUpdateLogic.cs:34`) runs `DestroyEntities()` then `KillEntities(dt)` each call — an entity killed this call only actually destroys (firing `_entityDestroySubscribers`, e.g. stat-rollback handlers) on the *next* call, and each call's kill cascades only one kill-tree level via `RequestKillChildren`. A forced synchronous drain outside the normal tick loop needs a loop until `KillRequest`/`DestroyRequest`/`DelayKillRequest` counts hit zero, not a single `Update(0f)` — and `DelayKillRequest` never resolves at `dt=0`, so use a generous per-iteration `dt` if the world isn't ticking to wait it out.
