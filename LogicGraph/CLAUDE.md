...
| Server→client transfer + **version gate** (#3.1) | **M6-E**: «версия кода» (`NodeContractHash`: имя+IL `Execute`+раскладка данных каждого `ILogicNode`) в `CompiledEnvironment`; `CompiledBlueprintStorage` рождается с окружением (`Create(CompiledEnvironment,…)`), гейт в `Add(arena,offsets,environment)` — окружение группы vs своё (`IsCompatibleWith`), несовместимая группа → `Dispose`+`throw`. Блоб о хеше не знает | ◐ **M6-E**: гейт на ingest реализован (off-chain, в пределах сборки). Transfer-time приём/сериализация конфига окружения — **M11**; content-digest из кодогена (вместо IL/layout-reflection) — **M10** |
| Burst functions compiled at startup, **by index** (#3.2) | dispatch-id `NodeHeader.typeId` = `TypeId<ILogicNode>` ordinal (M6-A); адаптер `NodeInvoker.Execute<T>`/`Compile<T>` → `FunctionPointer<ExecuteFn>` (M6-B); реестр `NodeFunctionRegistry.Build()` → таблица по ordinal (M6-C); прогон через `NodeInvoker.Invoke`/`Run` (M6-F) | ✅ **M6-A…F**: индекс закрыт; контракт исполнения (`NodeContext`+`ILogicNode.Execute`); реестр-инстанс (off-allocator `UnsafeArray<FunctionPointer>` + managed `ExecuteFn[]`); **M6-F** — прогон Drain-порядка через диспетчер (single-thread); per-node вызов `NodeInvoker.InvokeBurst` (blittable function-table, Burst) / `InvokeManaged`, ordinal — `Id<ExecuteFn>`. Параллелизм/wave — M7 |
| Dual backend: Burst **and** plain .NET, deterministic (#3.3) | `NodeInvoker.Execute<T>` (cross-env) + `Compile<T>` (Burst, `#if UNITY`) / `GetManaged<T>` (.NET); реестр строит обе таблицы (M6-C); выбор бэкенда по `runtimeType` (M6-D); диспатч в run'е (M6-F) | ✅ **M6-B…F**: единый адаптер в обе среды; реестр держит Burst-таблицу (`UnsafeArray<FunctionPointer>`, `#if UNITY` — `BurstTable`) + managed `ExecuteFn[]` (`ManagedTable`); выбор бэкенда `NodeFunctionRegistry.UseManaged(runtimeType)` — **раз/ноду в managed-glue** (`NodeInvoker.Invoke`), затем сразу нужная таблица-точка (`InvokeBurst`/`InvokeManaged`, без развилки внутри); реальное managed-исполнение/детерминизм-тесты; version gate — M6-E |
...
| **Orchestrator**: deps + parallel + Burst/non-Burst (#7) | `Orchestrator` (`Logic/RuntimeData/Execution/Orchestrator.cs`) — **демандный work-list** (M7-A/B/E); `NodeInvoker.Invoke` — per-node диспатч | ◐ **M7-A/B/E (демандная модель, 2026-06-18)**: `Inject(span)` сидит входы → `Run(scope, storage)` исполняет в порядке зависимостей (ready-по-предшественникам `relatives` + push + per-node мемоизация), **мульти-блюпринт** (per-instance `compiled`). **Статический батч-DAG/`ExecutionGraph`/`Drain` снесён.** Single-thread, тела eager-атомарны. **Lazy** (`Get`-yield, M7-D), **command-buffer** (M7-F), **параллелизм** (вне M7) — впереди. Авторитетная модель: `Plan/phase-M7/README.md` §«Целевая модель исполнения» |
