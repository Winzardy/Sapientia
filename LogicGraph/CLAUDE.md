...
| Dual backend: Burst **and** plain .NET, deterministic (#3.3) | `NodeInvoker.Execute<T>` (cross-env) + `Compile<T>` (Burst, `#if UNITY`) / `GetManaged<T>` (.NET); реестр строит обе таблицы (M6-C); выбор `NodeFunctionRegistry.UseManaged`/`Invoke` по `runtimeType` (M6-D) | ◐ **M6-B/C/D**: единый адаптер в обе среды; реестр держит Burst-таблицу (`UnsafeArray<FunctionPointer>`, `#if UNITY`) + managed `ExecuteFn[]`; **M6-D** — выбор бэкенда (`UseManaged`/`Invoke` по `runtimeType` + managed-форс), Burst-skip для `RuntimeType.Managed` (capability на `ILogicNode`, выводится в `INode<T>`), реальное managed-исполнение/детерминизм-тесты. Прогон `Drain` через scope — M6-F; version gate — M6-E |
