...
| **Orchestrator**: deps + parallel + Burst/non-Burst (#7) | `ExecutionGraph` (батч-DAG + `Drain`) + `NodeInvoker.Run` (run-prologue → Drain → per-node `Invoke`) | ◐ **single-thread прогон есть (M6-F)**: детерминированный обход зависимостей, тела нод исполняются. **Параллелизм + Burst/non-Burst wave-passes + мульти-блюпринтовый прогон — M7** (`NodeInvoker.Run` — временный seam, переедет) |
