...
**Статус: ✅ одобрено пользователем (коммитится).** Развилки: 2 (`AsyncValue`→синхронный `int`), 4 (Inject — runtime),
...
| # | Таска | Статус |
|---|---|---|
| 01 | [batch-DAG + Inject](tasks/01-inject.md) — типы, chain-декомпозиция, рёбра/счётчики | ✅ done |
| 02 | [drain + reset + dispose + cleanup](tasks/02-drain-dispose.md) — ready-queue обход; снос мёртвого каркаса | ✅ done |
| 03 | [tests](tasks/03-tests.md) — chain/diamond/parallel/multi-instance/reset/dispose/fan-out/repeat-drain | ✅ done |

> Self-review (Step 4): adversarial-сабагент → добавлены bounds-guard в `Drain`, страховка от цикла
> (`batchOf` −1 + guard в walk + assert'ы), тесты fan-out-из-хвоста и repeat-Drain-без-Reset. На ревью.
