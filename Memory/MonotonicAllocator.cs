...
namespace Sapientia.Memory
{
	...
	/// </summary>
	public struct MonotonicAllocator : IDisposable
	{
		private int _reservedSize;
		private SafePtr _memory;
		private PtrOffset _rover;

		public SafePtr<MonotonicAllocator> AllocatorPtr => _memory;

		public static SafePtr<MonotonicAllocator> Create(int reservedSize)
		{
			var result = new MonotonicAllocator();

			E.ASSERT(reservedSize > 0);
			result._reservedSize = reservedSize + TSize<MonotonicAllocator>.size;
			result._memory = MemoryExt.MemAlloc(reservedSize, ClearOptions.ClearMemory);
			result._rover = default;
			result.MemAlloc<MonotonicAllocator>(out _) = result;
		...
		public void CreateRelativeOffset(ref PtrOffset<MonotonicAllocator> relativeOffset)
		{
		...
		public static SafePtr<MonotonicAllocator> Deserialize(ref StreamBufferReader stream)
		{
			var allocator = new MonotonicAllocator();
	...
	{
		public static ref MonotonicAllocator GetRelativeAllocator(this ref PtrOffset<MonotonicAllocator> relativeOffset)
		{
			return ref ((SafePtr)relativeOffset.AsSafePtr() + relativeOffset).Value();
		}

		public static PtrOffset MemAlloc(this ref PtrOffset<MonotonicAllocator> relativeOffset, int size)
		{
			return relativeOffset.GetRelativeAllocator().MemAlloc(size);
		}

		public static PtrOffset MemAlloc(this ref PtrOffset<MonotonicAllocator> relativeOffset, int size, int count)
		{
			return relativeOffset.GetRelativeAllocator().MemAlloc(size, count);
		}

		public static PtrOffset<T> MemAlloc<T>(this ref PtrOffset<MonotonicAllocator> relativeOffset)
			where T : unmanaged
		{
			return relativeOffset.GetRelativeAllocator().MemAlloc<T>();
		}

		public static ref T MemAlloc<T>(this ref PtrOffset<MonotonicAllocator> relativeOffset, out PtrOffset<T> offset)
			where T : unmanaged
		{
			return ref relativeOffset.GetRelativeAllocator().MemAlloc(out offset);
		}

		public static PtrOffset<T> MemAlloc<T>(this ref PtrOffset<MonotonicAllocator> relativeOffset, int count)
			where T : unmanaged
		{
			return relativeOffset.GetRelativeAllocator().MemAlloc<T>(count);
		}

		public static SafePtr GetPtr(this ref PtrOffset<MonotonicAllocator> relativeOffset, PtrOffset offset)
		{
			return relativeOffset.GetRelativeAllocator().GetPtr(offset);
		}

		public static SafePtr<T> GetPtr<T>(this ref PtrOffset<MonotonicAllocator> relativeOffset, PtrOffset<T> offset)
			where T : unmanaged
		{
			return relativeOffset.GetRelativeAllocator().GetPtr<T>(offset);
		}

		public static ref T GetRef<T>(this ref PtrOffset<MonotonicAllocator> relativeOffset, PtrOffset<T> offset)
			where T : unmanaged
		{
			return ref relativeOffset.GetRelativeAllocator().GetRef<T>(offset);
		}

		public static ref T GetRef<T>(this ref PtrOffset<MonotonicAllocator> relativeOffset, PtrOffset offset)
			where T : unmanaged
		{
			return ref relativeOffset.GetRelativeAllocator().GetRef<T>(offset);
		}
